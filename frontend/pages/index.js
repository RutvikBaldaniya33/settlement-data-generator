...
const SUGGESTED_QUESTIONS = [
  "Analyze this batch and tell me what needs my attention.",
...
function ReviewModal({ result, onClose, onDecide, deciding, readOnly }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-sub">
          ...
          {readOnly && <span className="review-badge">Razorpay TEST preview · read-only</span>}
        </div>
...
export default function Home() {
  ...
  async function handleRazorpayReconcile() {
    ...
    try {
      const res = await fetch(`/api/razorpay/reconcile?count=${RAZORPAY_FETCH_COUNT}`);
      if (!res.ok) {
        let detail = null;
        try { detail = (await res.json()).detail; } catch (_) { /* no JSON body */ }
        const fallback = {
          400: "Razorpay TEST credentials are missing or invalid. Check RAZORPAY_KEY_ID / RAZORPAY_KEY_SECRET on the backend.",
          401: "Razorpay rejected the configured credentials (authentication failed).",
          502: "Could not reach Razorpay's API. Check the backend's network connection and try again.",
        }[res.status] || `Razorpay reconciliation failed (HTTP ${res.status}).`;
        setRazorpayError(detail || fallback);
        return;
      }
      const data = await res.json();
      if (!data || typeof data.summary !== "object" || !Array.isArray(data.results)) {
        setRazorpayError("Unexpected response from the Razorpay reconciliation endpoint.");
        return;
      }
      setBatch(data);
      showToast("Razorpay TEST batch created — now the active batch.");
    } catch (e) {
      setRazorpayError(`Could not reach the backend: ${e}`);
    } finally {
  ...
  return (
    <div className="container">
      ...
      <div className="toolbar">
        ...
        <button className="btn" onClick={handleRazorpayReconcile} disabled={razorpayLoading}>
          {razorpayLoading ? "Running Razorpay TEST reconciliation…" : "Run Razorpay TEST Reconciliation"}
        </button>
      </div>

      {razorpayError && (
        <div className="error-box" style={{ marginBottom: 16 }}>
          Razorpay TEST reconciliation failed — {razorpayError}
        </div>
      )}
      ...
      <div className="qa-panel">
        <div className="tabs" style={{ border: "none", marginBottom: 12 }}>
          <div className="tab active" style={{ borderBottom: "none", paddingLeft: 0 }}>AI Finance Controller</div>
        </div>
